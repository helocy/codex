...
- 内存优化：embedding 加载后释放 Python 浮点列表，RSS 从 4.4 GB 降至 ~1.1 GB
- PDF 两级缓存：L1 缓存 PdfReader 对象，L2 缓存页面文字，重复访问无需重新解析
- 原始文档查找优先使用数据库保存的文件路径（O(1)），跳过目录遍历（原 4.75s）
- 修复管理员统计/删除接口崩溃（VACUUM 不能在事务中执行导致 session 中止）
- 修复代理环境下登录挂起问题（start.sh 和 deploy.sh 生成的启动脚本均加入 NO_PROXY）
- 修复 socks:// 代理协议不兼容 httpx，自动规范化为 socks5://
- 部署优化：选择 API Embedding 时跳过 torch（~2 GB）安装；选择本地模型时预下载并展示进度
