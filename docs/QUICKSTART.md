...
打开 http://localhost:5173，点击顶部「⚙️ 配置」标签，填写：
...
> ⚠️ **重要**：更换 embedding 模型后，需要在「配置」页重置知识库并重新上传文档，因为不同模型的向量维度不兼容。
...
3. 如需使用新模型，需先在「配置」页重置知识库，重新上传文档
